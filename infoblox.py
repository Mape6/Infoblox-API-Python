...
# Mike Patterson <mpatters@uwaterloo.ca>
# Ryan Goggin
# Steve Scott
# all UW-IST-ISS
# 2015-2018
...
class Infoblox(object):
    """ Implements the following subset of Infoblox IPAM API via REST API
        ...
        get_lease_by_mac
        update_network
        get_ipv4_address_by_network
    """

    def __init__(self, iba_ipaddr, iba_user, iba_password, iba_wapi_version, iba_dns_view='default',
                 iba_network_view='default', iba_verify_ssl=False):
    ...
    def get_all_networkcontainers(self, max_results='1000'):
        """ Implements IBA REST API call to retrieve all network containers
        Returns hash table of networkcontainers
        :param max_results: maximum number of results to return
            (optional)
        """
        rest_url = 'https://' + self.iba_host + '/wapi/v' + \
            self.iba_wapi_version + '/networkcontainer?_max_results=' + max_results
        try:
            r = requests.get(url=rest_url, auth=(
                self.iba_user, self.iba_password), verify=self.iba_verify_ssl)
            r_json = r.json()
            if r.status_code == 200:
                if len(r_json) > 0:
                    ...
                    raise InfobloxNotFoundException("No networkcontainers found")
            else:
                if 'text' in r_json:
                    raise InfobloxNotFoundException(r_json['text'])
                else:
    ...
    def create_network(self, network, comment='', disable=False, extattrs_site=''):
        """ Implements IBA REST API call to create DHCP network object
        :param network: network in CIDR format
        :param comment: Comment to add
        :param disable: disabled for DHCP, default False
        :param extattrs_site: additional extensible attributes
        """
        rest_url = 'https://' + self.iba_host + \
            '/wapi/v' + self.iba_wapi_version + '/network'

        payload = {"network": network, "disable": disable, "network_view": self.iba_network_view}

        if len(comment) > 0:
            payload['comment'] = comment
        if len(extattrs_site) > 0:
            payload['extattrs'] = '{"extattrs": {"Site": {"value": ' + extattrs_site + '}}}'
...
### Bug? Getting cltt information only works with dynamic IPs, not static reservations.
    ...
    def get_ipv4_addresses_by_network(self, network):
        """ Implements IBA REST API call to get all ipv4 addresses by network
        Returns array of host names in FQDN associated within given network
        :param network: IPv4 network address
        """
        rest_url = 'https://' + self.iba_host + '/wapi/v' + self.iba_wapi_version + \
            '/ipv4address?network=' + network + '&network_view=' + self.iba_network_view
        try:
            r = requests.get(url=rest_url, auth=(
                self.iba_user, self.iba_password), verify=self.iba_verify_ssl)
            r_json = r.json()
